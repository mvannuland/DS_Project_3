---
title: "Pinus myc project example"
author: "Michael Van Nuland"
...
TotalBiomass.mod.N.resids <- lm(log(TotalBiomass) ~ log(Nitrogen), data = na.exclude(pinus.myc.dat))
TotalBiomass.mod.P.resids <- lm(log(TotalBiomass) ~ log(Phosphorus), data = na.exclude(pinus.myc.dat))
N.resids <- resid(TotalBiomass.mod.N.resids)  
P.resids <- resid(TotalBiomass.mod.P.resids)
pinus.myc.dat <- cbind.data.frame(na.exclude(pinus.myc.dat), N.resids, P.resids)
...
### 3. 3-D response surface plots
This code uses Plot.ly interactive graphing capabilities for visualizing 3D-surfaces of plant biomass growth simultaneously across the two-dimensional axes of critical soil resources (nitrogen and phosphorus).
```{r warning=FALSE}
...
One main goal of the project was to estimate the geometric volume of the 3D-surface plots and quantify the effects of the different fungal treatments on the resultant volumes. For this, I used the convex hull R function to measure the volume parameters for the response surface plots in each fungal treatment. Specifically, I used a randomized sub-sampling and bootstrapping approach (preserving matrix structure) to create null distributions and calculate error estimates for convex hull volumes of the plant growth response surfaces across the two-dimensional nutrient treatments (above). This allowed me to compare how different fungal symbiont treatments affected the overall size and shape of two-dimensional plant responses to nutrient limitation.
```{r warning=FALSE}
set.seed(1324)
